...
// GET /api/reviews-with-customers
app.get('/api/reviews-with-customers', async (req, res) => {
  try {
    ...
    if (rating) {
      const ratingsArray = rating.toString().split(',').map(Number); // Chuyển chuỗi "1,2,3" thành mảng [1, 2, 3]
      aggregationPipeline.unshift({
        $match: { Rating: { $in: ratingsArray } }
      });
    }

    const reviewsWithCustomers = await reviewCollection.aggregate(aggregationPipeline).toArray();

    logger.info('Fetched reviews with customer details successfully', { count: reviewsWithCustomers.length });
    res.json(reviewsWithCustomers);
  } catch (err) {
    logger.error('Error in GET /api/reviews-with-customers', { error: err.message });
    res.status(500).json({ error: 'Lỗi server khi lấy danh sách đánh giá.', details: err.message });
  }
});
...
app.delete('/api/reviews/:id', async (req, res) => {
  ...
  try {
    ...
    logger.info(`Đã xóa đánh giá với id ${id}`);
    res.status(200).json({ message: 'Xóa đánh giá thành công' });
  } catch (err) {
    logger.error('Lỗi khi xóa đánh giá', { error: err.message });
